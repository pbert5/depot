...
test('reset only deletes the current worker profile documents', async ({ page }) => {
  ...
  await page.evaluate(async (doc) => { await fetch('/api/rosters', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(doc) }); }, document('worker-a-document'));
