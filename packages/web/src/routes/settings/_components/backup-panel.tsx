...
const BACKUPS: Array<{ format: BackupFormat; label: string; path: string; type: string }> = [
  ...
  {
    format: 'yaml',
    label: 'Export all data as YAML',
    path: '/api/export.yaml',
    type: 'application/yaml;charset=utf-8'
  }
];

const errorMessage = (cause: unknown) =>
  cause instanceof Error ? cause.message : 'The backup could not be imported.';
...
const BackupPanel = () => {
  ...
  const exportBackup = async (backup: (typeof BACKUPS)[number]) => {
    ...
    try {
      ...
      downloadFile(
        `depot-backup-${isoTimestamp()}.${backup.format}`,
        await response.text(),
        backup.type
      );
    } catch (cause) {
  ...
  const inspectFile = async (files: File[]) => {
    ...
    try {
      ...
      const [rostersResponse, collectionsResponse] = await Promise.all([
        fetch('/api/rosters'),
        fetch('/api/collections')
      ]);
      if (!rostersResponse.ok || !collectionsResponse.ok)
        throw new Error('Could not check existing data. Please try again.');
      const existing = [
      ...
      setPreview({
        ...
        rosters: bundle.rosters.length,
        collections: bundle.collections.length,
        conflicts: getConflictIds(bundle, new Set(existing.map(({ id }) => id))).size
      });
    } catch (cause) {
  ...
  const importBackup = async () => {
    ...
    try {
      const response = await fetch(`/api/import?conflict=${policy}`, {
        ...
        body: JSON.stringify(preview.bundle)
      });
      ...
      setError(
        `Imported ${result.rosters ?? 0} roster${result.rosters === 1 ? '' : 's'} and ${result.collections ?? 0} collection${result.collections === 1 ? '' : 's'}.`
      );
    } catch (cause) {
    ...
    <section className="flex flex-col gap-2">
      ...
      <div className="surface-card flex flex-col gap-3 p-3">
        <p className="text-sm text-body">
          Download all rosters and collections as a portable backup, or restore one after reviewing
          it.
        </p>
        <div className="flex flex-wrap gap-2">
          ...
          <ImportButton
            label={busy === 'import' ? 'Reading…' : 'Import backup'}
            disabled={busy !== null}
            onFilesSelected={(files) => void inspectFile(files)}
            buttonTestId="import-backup-button"
            inputTestId="import-backup-input"
          />
        </div>
        {preview && (
          <div
          ...
          >
            ...
            <dl className="grid grid-cols-2 gap-2 text-sm text-body">
              ...
              <div>
                <dt className="text-subtle">Rosters</dt>
                <dd>{preview.rosters}</dd>
              </div>
              <div>
                <dt className="text-subtle">Collections</dt>
                <dd>{preview.collections}</dd>
              </div>
            </dl>
            <label
              className="flex flex-col gap-1 text-sm font-medium text-foreground"
              htmlFor="backup-policy"
            >
              When an ID already exists
            </label>
            <select
              id="backup-policy"
              className="h-11 rounded-sm border border-border-strong bg-surface-card px-2 text-sm"
              value={policy}
              onChange={(event) => setPolicy(event.target.value as ConflictPolicy)}
            >
              <option value="create">Create only (keep existing)</option>
              <option value="replace">Replace existing</option>
            </select>
            {preview.conflicts > 0 && (
              <p className="text-sm text-warning-fg">
                {policy === 'create'
                  ? `${preview.conflicts} matching item${preview.conflicts === 1 ? '' : 's'} already exist. Create will keep them and the API will reject this import.`
                  : `${preview.conflicts} existing item${preview.conflicts === 1 ? '' : 's'} will be replaced.`}
              </p>
            )}
            <Button
              type="button"
      ...
      <Drawer
        ...
        position="bottom"
        data-testid="backup-replace-confirmation"
        aria-label="Replace existing data confirmation"
      >
        <div className="flex flex-col gap-4 p-5">
          <Alert variant="warning" title="Replace existing data">
            <p className="text-sm">
              This will overwrite {preview?.conflicts ?? 0} existing item
              {preview?.conflicts === 1 ? '' : 's'}. This cannot be undone.
            </p>
          </Alert>
          <div className="flex justify-end gap-2">
            ...
            <Button
              ...
              data-testid="confirm-backup-replace"
            >
