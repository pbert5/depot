...
const valid = {
  ...
  formatVersion: 1,
  rosters: [{ id: 'r1', name: 'Roster', factionId: 'f1' }],
  collections: []
};
...
describe('backup import validation', () => {
  ...
  it('rejects unsupported versions, missing arrays, malformed documents, and unsafe tags', () => {
    expect(() => parseBackup(JSON.stringify({ ...valid, formatVersion: 2 }), 'json')).toThrow(
      /Unsupported/
    );
    expect(() => parseBackup(JSON.stringify({ ...valid, collections: undefined }), 'json')).toThrow(
      /collections/
    );
    expect(() =>
      parseBackup(JSON.stringify({ ...valid, rosters: [{ id: 'r1' }] }), 'json')
    ).toThrow(/name/);
    expect(() =>
      parseBackup(
        JSON.stringify({ ...valid, collections: [{ id: 'r1', name: 'C', factionId: 'f1' }] }),
        'json'
      )
    ).toThrow(/duplicate/i);
  ...
  it('recognizes picker formats and counts IDs that would be replaced', () => {
    ...
    expect(getConflictIds(parseBackup(JSON.stringify(valid), 'json'), new Set(['r1']))).toEqual(
      new Set(['r1'])
    );
  });
});
