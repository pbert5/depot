...
export const parseKeywordGrants = (
  description: string,
  sourceAbilityId?: string
): KeywordGrant[] => {
  ...
  const pattern =
    /(?:GAIN|HAVE|BECOME|ARE TREATED AS HAVING)\s+(?:THE\s+)?([A-Z][A-Z0-9 -]{1,40}?)(?:\s+KEYWORD)?(?=\b|[.,;])/g;
  let match: RegExpExecArray | null;
  while ((match = pattern.exec(text))) {
    const prefix = text.slice(0, match.index);
    const target = prefix.match(
      /(?:ALL|ANY|FRIENDLY|YOUR)?\s*([A-Z][A-Z0-9 '&-]{1,48}?)\s+UNITS?\b[^.]{0,120}$/
    );
    const targetKeyword = target?.[1]
      ?.replace(/\b(?:ALL|ANY|FRIENDLY|YOUR)\b/g, ' ')
      .replace(/\s+/g, ' ')
      .trim();
    const grantedKeyword = match[1].trim();
    if (targetKeyword && grantedKeyword)
      grants.push({ targetKeyword, grantedKeyword, sourceAbilityId, sourceText: description });
  }
  return grants;
};
...
export const getDetachmentKeywordGrants = (
  ...
  abilities.flatMap((ability) =>
    ability.keywordGrants?.length
      ? ability.keywordGrants.map((grant) => ({
          ...grant,
          sourceAbilityId: grant.sourceAbilityId ?? ability.id,
          sourceText: grant.sourceText ?? ability.description
        }))
      : parseKeywordGrants(ability.description, ability.id)
  );

export const getEffectiveKeywords = (
  datasheet: Pick<Datasheet, 'keywords'> & Partial<Pick<Datasheet, 'name'>>,
  abilities: Pick<DetachmentAbility, 'id' | 'description' | 'keywordGrants'>[] = []
): Keyword[] => {
  ...
  for (const grant of getDetachmentKeywordGrants(abilities)) {
    const target = normalize(grant.targetKeyword);
    if (target !== normalize(datasheet.name ?? '') && !effective.has(target)) continue;
    const key = normalize(grant.grantedKeyword);
    if (!effective.has(key))
      effective.set(key, {
        datasheetId: datasheet.keywords[0]?.datasheetId ?? '',
        keyword: grant.grantedKeyword,
        model: '',
