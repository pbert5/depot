...
async function requestProfile(req: IncomingMessage): Promise<string> {
  ...
  if (!id || !uuidPattern.test(id))
    throw Object.assign(new Error('invalid profile identity'), { status: 400 });
...
async function profile(id: string): Promise<Profile> {
  const result = await pool.query(
    'SELECT id, display_name, created_at, kind FROM users WHERE id = $1',
    [id]
  );
...
async function rows(kind: Kind, userId: string): Promise<Document[]> {
  const result = await pool.query(
    `SELECT document FROM ${kind} WHERE user_id = $1 ORDER BY updated_at DESC`,
    [userId]
  );
...
async function one(kind: Kind, id: string, userId: string): Promise<Document | null> {
  const result = await pool.query(`SELECT document FROM ${kind} WHERE user_id = $1 AND id = $2`, [
    userId,
    id
  ]);
...
async function put(
  ...
  doc: Document,
  userId: string,
  conflict: 'replace' | 'create' = 'replace'
) {
  const table = kind;
  if (conflict === 'create') {
    const existing = await client.query(`SELECT 1 FROM ${table} WHERE user_id = $1 AND id = $2`, [
      userId,
      doc.id
    ]);
...
async function handle(req: IncomingMessage, res: ServerResponse) {
  ...
  if (req.method === 'OPTIONS') {
    ...
    return json(res, 204, null, {
      ...
      'access-control-allow-headers': 'content-type, x-depot-profile-id'
    });
  }
  if (
    url.pathname === '/health' ||
  ...
  if (url.pathname === '/api/profiles' && req.method === 'GET') {
    ...
    const result = await pool.query(
      'SELECT id, display_name, created_at, kind FROM users ORDER BY created_at, id'
    );
    return json(
      res,
      200,
      {
        profiles: profilesFromRows(result.rows),
        active: await profile(activeId),
  ...
  if (url.pathname === '/api/profiles/active' && req.method === 'GET') {
    return json(res, 200, await profile(await requestProfile(req)), {
      'cache-control': 'no-store'
    });
  }
  if (url.pathname === '/api/profiles' && req.method === 'POST') {
    ...
    const displayName = validateProfileName(values.displayName);
    const kind = validateProfileKind(values.kind) as ProfileKind;
    const id = randomUUID();
    await pool.query('INSERT INTO users (id, display_name, kind) VALUES ($1, $2, $3)', [
      id,
      displayName,
      kind
    ]);
    res.setHeader(
      'set-cookie',
      `${profileCookie}=${encodeURIComponent(id)}; Path=/; HttpOnly; SameSite=Lax`
    );
  ...
  if (profileMatch) {
    ...
    if (profileMatch[2] === 'select' && req.method === 'POST') {
      ...
      await requestProfile({
        ...req,
        headers: { ...req.headers, 'x-depot-profile-id': id }
    ...
    if (!profileMatch[2] && req.method === 'PATCH') {
      ...
      if (requester !== id)
        throw Object.assign(new Error('profile may only rename itself'), { status: 403 });
      const body = await readBody(req);
      const displayName = validateProfileName(
        body && typeof body === 'object' ? (body as Record<string, unknown>).displayName : undefined
      );
      const result = await pool.query(
        'UPDATE users SET display_name = $1 WHERE id = $2 RETURNING id, display_name, created_at',
        [displayName, id]
      );
  ...
  if (match) {
    ...
    const id = match[2];
    if (req.method === 'GET')
      return json(res, 200, id ? await one(kind, id, userId) : await rows(kind, userId));
    if (req.method === 'DELETE' && id) {
      await pool.query(`DELETE FROM ${kind} WHERE user_id=$1 AND id=$2`, [userId, id]);
      return json(res, 204, null);
    }
    if ((req.method === 'POST' && !id) || (req.method === 'PUT' && !!id)) {
      ...
      try {
        ...
        await put(
          ...
          userId,
          req.method === 'POST'
            ? ((url.searchParams.get('conflict') as 'replace' | 'create') ?? 'replace')
            : 'replace'
        );
  ...
  if (url.pathname === '/api/export' && req.method === 'GET')
    return json(res, 200, bundle(await rows('rosters', userId), await rows('collections', userId)));
  const singleExport = url.pathname.match(
    /^\/api\/(rosters|collections)\/([^/]+)\/export(?:\.yaml)?$/
  );
  if (singleExport && req.method === 'GET') {
    ...
    if (url.pathname.endsWith('.yaml'))
      return text(res, 200, stringifyYaml(payload), {
  ...
  if (url.pathname === '/api/import' && req.method === 'POST') {
    ...
    const parsed =
      typeof raw === 'object' && raw && 'content' in raw
        ? (raw as { content: string; format?: string })
        : raw;
    let source: unknown;
    try {
      source =
        typeof parsed === 'string'
          ...
          : typeof parsed === 'object' &&
              parsed &&
              'format' in parsed &&
              (parsed as { format?: string }).format === 'yaml'
    ...
    if (
      ...
      (source as Record<string, unknown>).format !== format ||
      (source as Record<string, unknown>).formatVersion !== schemaVersion
    )
      throw new Error('unsupported depot export format or version');
    const input = source as { rosters?: unknown; collections?: unknown };
    if (!Array.isArray(input.rosters) || !Array.isArray(input.collections))
      throw new Error('export must contain rosters and collections arrays');
    const rosters = input.rosters.map(validateDocument);
    const collections = input.collections.map(validateDocument);
    const requestedPolicy = url.searchParams.get('conflict') ?? 'replace';
    if (requestedPolicy !== 'create' && requestedPolicy !== 'replace')
      throw new Error('conflict policy must be create or replace');
    const policy = requestedPolicy;
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      for (const doc of rosters) await put(client, 'rosters', doc, userId, policy);
      for (const doc of collections) await put(client, 'collections', doc, userId, policy);
      await client.query('COMMIT');
      return json(res, 200, { rosters: rosters.length, collections: collections.length });
    } catch (error) {
      await client.query('ROLLBACK');
      throw error;
    } finally {
  ...
  if (url.pathname === '/api/export.yaml' && req.method === 'GET')
    return text(
      res,
      200,
      stringifyYaml(bundle(await rows('rosters', userId), await rows('collections', userId))),
      { 'content-type': 'application/yaml; charset=utf-8' }
    );
...
const ensureDefaultProfile = async () => {
  await pool.query(
    'INSERT INTO users (id, display_name, kind) VALUES ($1, $2, $3) ON CONFLICT (id) DO NOTHING',
    [
      defaultUserId,
      defaultUserId === '00000000-0000-0000-0000-000000000001' ? 'Local User' : 'Depot User',
      'main'
    ]
  );
};
...
const server = createServer((req, res) => {
  void handle(req, res).catch((error: unknown) =>
    json(res, Number((error as { status?: number }).status ?? 400), {
      error: error instanceof Error ? error.message : 'request failed'
    })
  );
});
void ensureDefaultProfile()
  .then(() => server.listen(port, '0.0.0.0', () => console.log(`Depot API listening on ${port}`)))
  .catch((error) => {
    console.error('Unable to seed default profile', error);
    process.exitCode = 1;
  });

export { bundle, profileFromRow, profilesFromRows, validateDocument, validateProfileKind };
